...
`env.mail*`, пароль — секрет `VERAKS_MAIL_PASSWORD` выше). `env.mailHost` по
умолчанию — пустой плейсхолдер, и это **не fail-fast**: в отличие от
платёжных провайдеров, отсутствие SMTP не роняет старт бэкенда (email сейчас
единственный включённый способ входа, падение старта означало бы
недоступность всей платформы). Без `mailHost` письма не доставляются
пользователям — бэкенд пишет письмо целиком, включая ссылку входа, в лог
ERROR-уровня (`kubectl -n veraks logs deploy/veraks-backend`); этого
достаточно, чтобы оператор вручную провёл вход или проверил работу e-mail-
потока. Как только появится реальный SMTP-провайдер — заполнить
`env.mailHost`/остальные `env.mail*`/секрет `VERAKS_MAIL_PASSWORD` через
`helm upgrade --set` (или правкой `values.yaml`) в любой момент, без
пересборки образа, и письма начнут доставляться по-настоящему.
