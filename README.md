...
### Хранилище БД (важно для демо)
По умолчанию Postgres — **эфемерный** (`emptyDir`, `postgres.persistence: false`):
кластер aerolist не имеет провижнера/StorageClass, а демо-данные восстанавливает
seed. Данные не переживают пересоздание пода Postgres (перезапуск ноды, смена
образа). Для долговечной БД в этом же кластере:
1. поставить провижнер, напр. `kubectl apply -f https://raw.githubusercontent.com/rancher/local-path-provisioner/v0.0.30/deploy/local-path-storage.yaml`
   (создаёт cluster-scoped StorageClass — согласовать, кластер общий с aerolist);
2. в `values.yaml`: `postgres.persistence: true` и `postgres.storageClassName: local-path`;
3. пересоздать StatefulSet (том у него immutable): `kubectl -n veraks delete statefulset veraks-postgres` перед деплоем.

Боевой контур позже — managed-Postgres с бэкапами (см. ниже).
